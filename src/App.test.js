...
import {render, wait, getByPlaceholderText, getByTestId} from '@testing-library/react'
import App from './App'
...
const episodes = {
  data: [
    ...
    test('APP fetches season and render data', async () => {
      mockFetchShow.mockResolvedValue(episodes)
      const {getByText, queryAllByTestId, getByPlaceholderText} = render(<App/>)
  
      const options = getByPlaceholderText(/Select an option/)
      eventUser(options)
  
      getByText(/fetching data.../i)
      await wait(()=> {
        getByTestId('episode-map')
      })
      expect(queryAllByTestId(eps)).toBeInTheDocument
  
  })
  
  

test('Change of season select season', () => {
  const mockGetData = jest.fn();
    const {getByText, queryByText, rerender} = 
    render(<App fetchShow={mockGetData} isFetchingData={true}/>)
    getByText(/fetching data.../i)
    expect(queryByText(/select a season/i)).toBeNull

    rerender(<App fetchShow={mockGetData} isFetchingData={false}/>)

    queryByText(/select a season/i)
